...
export function CashflowChart({
  ...
  // y-axis gridlines: 4 round steps
    ...
    >
      ...
      {forecast.length > 0 && (
        <>
          ...
          <text x={splitX + 6} y={padT + 6} fontSize="9.5" fill="#8A6323" fontWeight="600" letterSpacing="1.2" fontFamily="var(--font-sans)">
            FORECAST AT STRIP
          </text>
        </>
      )}
      ...
      <text x={x(0) + 2} y={H - 8} fontSize="10" fill="#71817A" fontFamily="var(--font-mono)">
        {monthLabel(all[0].month)}
      </text>
      {forecast.length > 0 && (
        <text x={splitX + 4} y={H - 8} fontSize="10" fill="#8A6323" fontFamily="var(--font-mono)">
          {monthLabel(forecast[0].month)}
        </text>
      )}
      <text x={W - 6} y={H - 8} textAnchor="end" fontSize="10" fill="#71817A" fontFamily="var(--font-mono)">
